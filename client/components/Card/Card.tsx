...
const Card: any = ({ title, row }: any) => {
  ...
  const {
    ...
    "trend",
    async (a) => {
      const data = await baseurl.get("/trending", {
        params: { sort: "trendeingScore:desc", curser: a.pageParam },
      });
      return data.data;
    },
    {
  ...
  const scroll = () => {
    if (data) {
      return data.pages.map((page) => {
        return page.trending.map((item: any, i: number) => {
          if (i === 4) {
            return (
              <div ref={ref} key={item.id} className={style.scrollItem}>
                ...
                <div className={style.right}>
                  <h6>{moment(item.cratedAt).fromNow()}</h6>
                  <div>
                    <h5>{item._count.likes}</h5>
                    <ThumbsUp size={20} />
                  </div>
                </div>
              </div>
